...
class TMP117TemperatureSensor : public GPIOPin::GPIOEventCallbackInterface {
	...
	virtual void hasEvent(gpiod_line_event& e) override {
		...
		printf("TMP117 Sensor: interrupt received!\n");

	    switch (e.event_type) {
	    case GPIOD_LINE_EVENT_RISING_EDGE:
		{
		printf("TMP117 Sensor: Rising event!\n");

		double temperature = readTemperature();

		if (std::isnan(temperature)) {
			//printf("Sensor %d : Failed to read temperature.\n", sensor_id);
			printf("TMP117 Sensor: Failed to read temperature.\n");
		} else {
			//printf("Sensor %d : 🌡️ Temperature: %.2f °C\n", sensor_id, temperature);
			printf("TMP117 Sensor: 🌡️ Temperature: %.2f °C\n", temperature);
		}
	    ...
	    default:
			printf("TMP117 Sensor: Unkown event\n");
	    }
	}
	/**
	...
	double readTemperature() {
		...
		printf("TMP117 Sensor: waiting for I2C lock.\n");
